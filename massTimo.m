...
for i = 1:n
   M(2*i-1,2*i-1) = h*((i<n)*int_phi(1,1)+(i>1)*int_phi(3,3)); 
   M(2*i-1,2*i) = h^2*((i<n)*int_phi(1,2)+(i>1)*int_phi(3,4));
   M(2*i,2*i) = h^3*((i<n)*int_phi(2,2)+(i>1)*int_phi(4,4));
   if (i<n)
   M(2*i,2*i+1) = h^2*int_phi(2,3);
   M(2*i,2*i+2) = h^3*int_phi(2,4);
   M(2*i-1,2*i+1) = h*int_phi(1,3);
   M(2*i-1,2*i+2) = h^2*int_phi(1,4);
   end
   
end

M = mu*M;
M = M+M'-diag(diag(M)); % Macht S symmetrisch voll
